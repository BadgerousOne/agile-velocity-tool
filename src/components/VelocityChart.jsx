...
export default function VelocityChart() {
  ...
  const [compactMode, setCompactMode] = useLocalStorageState(
    'vel_chart_compact_mode',
    true,
    { parse: v => v === 'true', serialize: v => String(v) }
  );
  const [activeChartId, setActiveChartId] = useLocalStorageState(
    'vel_chart_active',
    'velocity-trend'
  );
  ...
  const burnupData = useMemo(() => {
    ...
    return sprints.map(s => {
      ...
      return { name: s.name, cumulativeCompleted, cumulativeScope };
    });
  }, [sprints]);

  const capacityData = useMemo(() => sprints.map(s => {
    const t = sprintCapacityTotals(s);
    return { name: s.name, ptoDays: t.ptoDays, supportDays: t.supportDays, otherDays: t.otherDays, completed: s.completedPoints };
  }), [sprints]);

  const utilizationData = useMemo(() => chartData.map(d => ({
    name: d.name, utilization: d.utilization, effectiveFTEs: d.effectiveFTEs,
    completed: d.completed, adjVelocity: d.adjVelocity,
  })), [chartData]);
  ...
  // Chart definitions — id must be stable (used as localStorage key value)
  const charts = [
    {
      id: 'velocity-trend',
              ...
              <Bar yAxisId="left" dataKey="utilization" name="Utilization %" radius={[4,4,0,0]}>
                {utilizationData.map((d, i) => (
                  <Cell key={i} fill={
                    ...
                    d.utilization >= 70   ? 'var(--warning)' : 'var(--danger)'
                  } />
                ))}
              </Bar>
    ...
    },
    {
      ...
      render: () => (
        <div className="card chart-section">
          ...
          <p className="chart-sub">Completed line should converge with Scope as the project progresses. A widening gap signals scope creep.</p>
          <ResponsiveContainer width="100%" height={240}>
            <ComposedChart data={burnupData}>
              <CartesianGrid strokeDasharray="3 3" stroke="var(--border)" />
              <XAxis dataKey="name" tick={tickStyle} />
              <YAxis tick={tickStyle} />
              <Tooltip contentStyle={tooltipStyle} labelStyle={labelStyle} itemStyle={itemStyle} formatter={(val, name) => [`${val} pts`, name]} />
              <Legend wrapperStyle={legendStyle} />
              <Line dataKey="cumulativeScope"     name="Cumulative Scope"     type="monotone" stroke="var(--warning)" strokeWidth={2} strokeDasharray="5 3" dot={{ r: 3, fill: 'var(--warning)' }} />
              <Line dataKey="cumulativeCompleted" name="Cumulative Completed" type="monotone" stroke="var(--success)" strokeWidth={2} dot={{ r: 4, fill: 'var(--success)' }} />
            </ComposedChart>
          </ResponsiveContainer>
        </div>
      ),
        ...
        <div className="card chart-section">
          ...
          <div className="vel-table-wrap">
            <table className="vel-table">
              <thead>
                <tr>
                  <th>Sprint</th><th>Committed</th><th>Completed</th><th>% Done</th>
                  <th>Eff. FTEs</th><th>Utilization</th><th>Adj. Velocity</th>
                  <th>🏖 PTO</th><th>🛠 Support</th><th>📌 Other</th><th>Rolling Avg</th>
                </tr>
              </thead>
              <tbody>
                {sprints.map((s, i) => {
                  const pct      = s.committedPoints > 0 ? Math.round((s.completedPoints / s.committedPoints) * 100) : 0;
                  ...
                  const utilColor = cd?.utilization == null ? 'var(--text-muted)'
                    : cd.utilization >= 90 ? 'var(--success)' : cd.utilization >= 70 ? 'var(--warning)' : 'var(--danger)';
                  return (
                    <tr key={s.id || s.name}>
                      <td>{s.name}</td>
                      ...
                      <td style={{ color: 'var(--primary-light)', fontWeight: 600 }}>{totals.effectiveFTEs ?? '—'}</td>
                      <td style={{ color: utilColor, fontWeight: 600 }}>{cd?.utilization != null ? `${cd.utilization}%` : '—'}</td>
                      <td style={{ color: 'var(--success)', fontWeight: 600 }}>{cd?.adjVelocity != null ? `${cd.adjVelocity} pts` : '—'}</td>
  ...
  return (
    <div className="velocity-page">
      <div className="page-header">
        ...
        {/* View mode toggle */}
        <div className="vel-view-toggle">
          <button
            className={`vel-view-btn ${compactMode ? 'active' : ''}`}
            title="Show one chart at a time"
      ...
      {compactMode ? (
        /* ── Compact: tab strip + single chart ── */
        <>
          <div className="vel-chart-tabs">
            {charts.map(c => (
              <button
                key={c.id}
                className={`vel-chart-tab ${activeChart.id === c.id ? 'active' : ''}`}
        ...
        /* ── Expanded: all charts stacked ── */
        charts.map(c => <React.Fragment key={c.id}>{c.render()}</React.Fragment>)
      )}
    </div>
  );
}
